...
function Auth({ onLogin }) {
  ...
  const login = async () => {
    ...
    if (cleanPhone.length !== 11 || cleanPhone[0] !== '7') return setError('Введите номер телефона в формате +79271234567');
    setLoading(true);
    const { data, error } = await supabase.auth.signInWithPassword({ phone: `+${cleanPhone}`, password });
    setLoading(false);
    if (error) return setError(error.message === 'Invalid login credentials' ? 'Неверный номер телефона или пароль' : error.message);
    onLogin(data.user);
  };
  return <main className="app auth-page"><section className="card auth-card"><h1>Путёвка</h1><p>Вход в приложение</p><label>Номер телефона</label><input type="tel" inputMode="tel" autoComplete="tel" value={phone} onChange={e => setPhone(formatPhone(e.target.value))} onKeyDown={e => e.key === 'Enter' && login()} placeholder="+79271234567"/><label>Пароль</label><input type="password" autoComplete="current-password" value={password} onChange={e => setPassword(e.target.value)} onKeyDown={e => e.key === 'Enter' && login()} placeholder="Введите пароль"/>{error && <div className="error">{error}</div>}<button className="primary big" disabled={loading || phone.replace(/\D/g, '').length !== 11 || !password} onClick={login}>{loading ? 'Вход...' : 'Войти'}</button></section></main>;
}
...
function App({ user, profile, onLogout }) {
  ...
  if (screen === 'home') return <main className="app"><header><div><h1>Путёвка</h1><p>{profile?.full_name || user.phone}</p></div><div className="header-actions">{profile?.role === 'admin' && <button className="icon" onClick={() => setScreen('admin')}>⚙️</button>}<button className="icon" onClick={onLogout}>↪</button></div></header><section className="card profile"><span>👤</span><div><b>{profile?.role === 'admin' ? 'Администратор' : 'Водитель'}</b><span>{profile?.full_name || user.phone}</span></div></section><section className="card"><label>🚗 Автомобиль</label><select value={carId} onChange={e => setCarId(e.target.value)}>{cars.map(c => <option value={c.id} key={c.id}>{c.name}</option>)}</select></section><button className="primary big" onClick={() => setScreen('waybill')}>＋ Новая путёвка</button></main>;
